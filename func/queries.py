...
def display_halt_times():
    ...
    ABS(TIME_TO_SEC(TIMEDIFF(ts.DepartureTime, ts.ArrivalTime))) AS HaltTime
    FROM TrainStops ts
    JOIN Trains t ON ts.TrainNo = t.TrainNo
    ORDER BY HaltTime DESC LIMIT 10;
    '''
    cursor.execute(query)
    longest_halt_times = cursor.fetchall()
    ...
    query = '''
        ...
        FROM TrainStops ts
        JOIN Trains t ON ts.TrainNo = t.TrainNo
        ORDER BY HaltTime ASC LIMIT 10;
        '''
    cursor.execute(query)
    shortest_halt_times = cursor.fetchall()

    return longest_halt_times, shortest_halt_times, cursor.description
